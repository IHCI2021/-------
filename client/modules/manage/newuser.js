...
getusername(e){
  ...
  formSubmit(){
	...
	axios({
		method:'post',
		url:"http://10.147.15.189:8000/api/user/create",
		data:data,
        ...
        {typeof window!=="undefined"?(localStorage.getItem("username")=='root'?(
          <div  onClick={() => this.touserManage()}
			...
			<div style={mainArea}>
				...
				<div style={nameArea}>
					<div style={loginTest}>用户创建</div>
				</div>

				<div style={keyinArea}>
					<div style={firstName}>用户名：</div>
				    <div style={secondName} >密码:</div>
					...
					style={area1} type="text" placeholder="请输入用户名" v-model="usr"></input>
					<input
					value={this.state.password}
					onChange={this.getpassword.bind(this)}
					style={area2} type="password" placeholder="请输入密码" v-model="usr"></input>
                    <input
					value={this.state.confirmpwd}
					onChange={this.getconfirmpwd.bind(this)}
					style={area3} type="password" placeholder="请确认密码" v-model="usr"></input>
				</div>
