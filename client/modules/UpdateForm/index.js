...
toindex(){
  ...
  getItem(){
    let that = this;
      axios.get('http://10.147.15.189:8000/api/material/getItemByCode', {
        headers: {
...
inputJudge(page) {
  if (page == 0) {
    ...
    else if (!this.state.item_code.match('^[A-Z0-9]{31}$')) {
  ...
  prev() {
   ...
   //表单提交
    formSubmit(){
      ...
      if(!this.state.item_code.match('^[A-Z0-9]{31}$')){
    ...
    for(var i=0;i<tmpMat.length;i++){
     ...
     for(var i=0;i<tmpAdd.length;i++){
      ...
      axios({
        method:"post",
        url:"http://10.147.15.189:8000/api/material/updateItem",
        data:data,
  ...
  localStorage.setItem('username', "");
        ...
        if(err.response.status==403){
          alert(err.response.data.msg)
          //window.location.href = 'http://10.147.15.189:8000/test/post_material_form'
          this.props.router.push('/index');
        }
      ...
      //        window.location.href = 'http://10.147.15.189:8100/test/post_material_form'
      //        }
      //      }
       ...
       {this.state.current === 0 && (
        ...
        <div className="steps-action">
          ...
          {this.state.current === steps.length - 1 && (
            ...
            //onClick={()=>{window.location.href="http://10.147.15.189:8100/test/post_material_form"}}
            >
              提交
            </Button>
